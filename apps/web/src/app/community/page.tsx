      ...
      <div className="space-y-6">
        <div className="product-card">
          <div className="nav-pill-group">
            <button onClick={() => setSelectedCategory('')} className={!selectedCategory ? 'nav-pill-active' : 'nav-pill'}>
              All
            </button>
            {CATEGORIES.map((cat) => (
              <button key={cat} onClick={() => setSelectedCategory(selectedCategory === cat ? '' : cat)} className={selectedCategory === cat ? 'nav-pill-active' : 'nav-pill'}>
        ...
        <div className="space-y-4">
          {reports.map((report) => {
            ...
            return (
              <div key={report.id} id={`report-${report.id}`} className="product-card scroll-mt-24 space-y-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-lg font-semibold text-ink">{report.title ?? cat?.label ?? report.category}</p>
                ...
                <div className="flex flex-wrap items-center gap-2 text-xs">
                  <span className={report.pinColor === 'red' ? 'badge-emergency' : report.pinColor === 'yellow' ? 'badge-warning' : 'eyebrow bg-surface-soft'}>Pin {report.pinColor}</span>
                  <span className="eyebrow bg-surface-soft">Score {report.score}</span>
                  <span className="eyebrow bg-surface-soft">{report.upvoteCount} likes</span>
                  <span className="eyebrow bg-surface-soft">{report.commentCount} comments</span>
                </div>
                ...
                {isAuthenticated ? (
                  <div className="rounded-xl border border-hairline p-4">
                    ...
                    <textarea value={draft} onChange={(event) => setDraftComments((current) => ({ ...current, [report.id]: event.target.value }))} rows={2} className="textarea-field mt-3 min-h-20 resize-none" placeholder="Add corroborating detail for this location..." />
