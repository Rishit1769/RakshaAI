...
export const registerSchema = z.object({
  ...
  role: z
    .enum(['user'], {
      errorMap: () => ({ message: 'Self-registration is only allowed for the user role' }),
    })
    .optional()
    .default('user'),
});
...
export const loginSchema = z.object({
  credential: z
    .string({ required_error: 'Email or MPIN is required' })
    .trim()
...
export const setupMpinSchema = z.object({
  mpin: z
    ...
    .trim()
    .regex(/^\d{4,6}$/, 'MPIN must be 4 or 6 digits'),

  confirmMpin: z
    .string({ required_error: 'Please confirm your MPIN' })
    .trim(),
